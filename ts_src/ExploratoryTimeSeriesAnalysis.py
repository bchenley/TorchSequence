        ...
        for iy in range(int(np.sqrt(len(self.xcorr)))):
          ...
          if any(f"{name}[t]" in xcorr_names[i] for name in data_names):
            ...
            if domain == 'frequency':
              ...
              xlabel = f"Frequency [1/{self.time_unit}]"
              if self.data_units is not None:
                ylabel = ylabel = f"${self.data_units[ix]}^2\cdot{self.time_unit}$" if ix == iy else f"${self.data_units[ix]}\cdot{self.data_units[iy]}/\mathrm{Hz}$"
            else:
              xaxis = self.lags
              xlabel = f"Lags [{self.time_unit}]"
              if self.data_units is not None:
                ylabel = f"${self.data_units[ix]}^2$" if ix == iy else f"${self.data_units[ix]}\cdot{self.data_units[iy]}$"
