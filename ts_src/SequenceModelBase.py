...
class SequenceModelBase(torch.nn.Module):
  ...
  def __init__(self,
              ...
              lru_input_block_weights_to_1 = False,
              ...
              cnn_activation = ['identity'],
              cnn_degree = [2], cnn_coef_init = [None], cnn_coef_train = [True], cnn_coef_reg = [[0.001, 1]] , cnn_zero_order = [False], 
              cnn_dropout_p = [0.],
    ...
    if self.base_type == 'identity':
      ...
      self.base = LRU(input_size = self.input_size, 
                      ...
                      input_block_weights_to_1 = lru_input_block_weights_to_1,
                      device = self.device, dtype = self.dtype)
      ...
      self.base = CNN1D(in_channels = self.input_size, 
                        ...
                        degree = self.cnn_degree,
                        coef_init = self.cnn_coef_init, 
                        coef_train = self.cnn_coef_train, 
                        coef_reg = self.cnn_coef_reg, 
                        zero_order = self.cnn_zero_order, 
                        pool_type = self.cnn_pool_type, 
                        pool_size = self.cnn_pool_size,
                        pool_stride = self.cnn_pool_stride,
                        batch_norm = self.cnn_batch_norm, 
                        batch_norm_learn = self.cnn_batch_norm_learn,
                        dropout_p = self.cnn_dropout_p,
                        device = self.device, dtype = self.dtype)
    ...
    with torch.no_grad():            
      X = torch.empty((2, self.input_len, input_size)).to(device = self.device,
                                                          dtype = self.dtype)
      encoder_output = torch.empty((2,self.input_len,encoder_output_size)).to(device = self.device,
