    ...
    for arg in locals_:
      ...
      if isinstance(value, list) and any(x in arg for x in ['seq_type', 'input_size', 'base_', 'decoder_', 'hidden_', 'attn_']):  
        if len(value) == 1:  
          ...
          locals_[arg] = locals_[arg] * num_inputs # setattr(self, arg, value * num_inputs) # exec(f"{arg} = {arg} * num_inputs")

      elif isinstance(value, list) and any(x in arg for x in ['output_size', 'output_']):        
        if len(value) == 1:
          locals_[arg] = locals_[arg] * num_outputs # setattr(self, arg, value * num_outputs) # exec(f"{arg} = {arg} * num_outputs")
